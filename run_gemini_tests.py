"""
MCP Client Test Script for Gemini CLI
...
def build_report(results, start_time, end_time):
    ...
    lines = [
        f"# MCP Client Test Report — Gemini CLI",
        f"",
        f"**Date:** {date_str}  ",
        ...
        f"| Field | Value |",
        f"|-------|-------|",
        f"| Client | Gemini CLI (gemini-2.5-pro) |",
        f"| Platform | macOS Darwin 25.3.0 (arm64) |",
        f"| Test date | {date_str} |",
        f"| Script | run_gemini_tests.py |",
    ...
    if failures:
        ...
        lines.append(
            "All 10 tests passed. "
            "The mcp-macos-colima server is fully functional when accessed via the Gemini CLI MCP client."
        )
...
async def main():
    print("=== mcp-macos-colima — Gemini CLI MCP Client Tests ===")
    ...
    date_str = start_time.strftime("%Y-%m-%d")
    report_path = f"test_reports/mcp_colima/gemini/mcp-client-results-{date_str}.md"
    ...
    json_path = f"test_reports/mcp_colima/gemini/raw-results-{date_str}.json"
